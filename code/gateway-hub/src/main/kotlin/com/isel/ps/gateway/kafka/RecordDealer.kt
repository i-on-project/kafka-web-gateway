...
import com.isel.ps.gateway.websocket.HubWebsocketHandler.Companion.json
...
class RecordDealer(
    ...
    init {
        val gatewayInstance = hubConfig.getGateway()
        consumer =
            KafkaConsumer(kafkaClientsUtils.getConsumerDefaultProperties(gatewayIdLongToConsumerIdString(gatewayInstance.hubId)))
    ...
    private fun updateRequestedKeysTopic(topic: String) {
        val record = ProducerRecord(
            hubConfig.getGateway().topicKeys,
            topic,
    ...
    private fun getAllTopicKeys(topic: String): HubTopicKeys {
        return if (fullTopics.containsKey(topic)) {
            HubTopicKeys(null, true)
        } else {
            HubTopicKeys(
